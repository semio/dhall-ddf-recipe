      ...
      → merge
          { t1 =
              ...
              → P.filter.run
                  ...
                  , result = "dps-" ++ r.id ++ "-filtered"
                  , options =
                    { item = P.filter.item.simple [ r.indicator ]
                    , row =
...
let flattenProc =
      ...
      → P.flatten.run
          { ingredients = [ ingredient ]
          , result = ingredient ++ "-flattened"
          , options =
            { dictionary = [ { mapKey = k, mapValue = v } ]
            , flatten_dimensions = dims
...
let groupbyProc =
      ...
      → P.groupby.run
          { ingredients = [ ingredient ]
          , result = ingredient ++ "-aggregated"
          , options =
            { groupby = [ "cause", "sex", "location", "year" ]
            , aggregate = [ { mapKey = indicator, mapValue = "sum" } ]
            }
          }
...
let getResult =
        λ(rec : Record)
      → merge
          { t1 = λ(r : Rec.t1) → "dps-" ++ r.id ++ "-filtered-flattened"
          , t2 =
...
let merge_result =
      [ P.merge.run
          { ingredients = results_1 # results_2
